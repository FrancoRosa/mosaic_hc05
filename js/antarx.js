...
// set once connectGPS() runs and apian.js hands us its broadcaster; until
...
// AntaRx-Si3 reports over a USB virtual serial port, same as Mosaic-H's
// "Septentrio" manufacturer string; matching "antarx" too in case its
// descriptor differs — check the ports list in the UI if neither matches
const isReceiverPort = (port) =>
  /septentrio|antarx/i.test(port.manufacturer || "");

const getReceiverPorts = async (broadcaster) => {
  let ports = await SerialPort.list();
  broadcaster("ports", {
    ports: ports.filter((port) => port.manufacturer !== undefined),
  });
  return ports.filter(isReceiverPort).map((p) => p.path);
};
...
const connectGPS = async (broadcaster) => {
  ...
  paths = await getReceiverPorts(broadcaster);
